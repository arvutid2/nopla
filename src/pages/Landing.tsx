import AuthProbe from "../components/debug/AuthProbe";
...
// (eemaldasime SupaDebugi; kui tahad, jäta alles)
...
export default function Landing() {
  ...
  // PVP match info
  const [pvp, setPvp] = useState<{ matchId: string; role: "host" | "guest" } | null>(null);

  // sessiooni key – iga mängu jaoks puhas GameOverlay
    ...
    <div className="relative min-h-screen w-full overflow-clip bg-[#060b1a] text-white">
          ...
          >
            <div>
              ...
              <p className="mt-4 text-white/70 md:text-lg">
                Instant 1v1 duels. Secure escrow with Pi. Neon-lit space
                vibes, buttery animations, zero friction.
              </p>
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                >
                  <Pill icon={Gamepad2} variant="accent">
                    Find 1v1 Match
                  </Pill>
                </a>
                <Pill icon={ShieldCheck} variant="ghost">
                  Provably fair
                </Pill>
              </div>
            ...
            >
              ...
              <div className="relative flex h-full w-full items-center justify-center rounded-[36px] border border-white/10 bg-white/[0.04] backdrop-blur-xl">
                <motion.div
                  ...
                  style={{ boxShadow: "0 0 60px rgba(99,102,241,.25) inset" }}
                />
                <motion.div
                  animate={{ rotate: -360 }}
                  transition={{ repeat: Infinity, duration: 40, ease: "linear" }}
                  className="absolute h-[55%] w-[55%] rounded-full border border-white/10"
                  style={{ boxShadow: "0 0 40px rgba(34,211,238,.25) inset" }}
                />
          ...
          <div className="grid grid-cols-1 items-center gap-6 md:grid-cols-3">
            <div className="md:col-span-2">
              ...
              <p className="mt-1 text-white/70">
                Create a lobby, set a buy-in and invite a friend.
              </p>
            </div>
            <div className="flex flex-wrap items-center justify-start gap-3 md:justify-end">
              <Pill icon={ChevronRight} variant="primary" onClick={scrollToMatch}>
                Get started
              </Pill>
            </div>
          </div>
      ...
      <MatchmakingOverlay
        ...
        onFound={(p) => {
          ...
          // PVP
          // @ts-ignore
          setPvp({ matchId: p.matchId, role: p.role });
          setGameOpen(true);
          setSessionId((s) => s + 1);
        }}
      />
      ...
      <GameOverlay
        ...
        matchId={pvp?.matchId}
        role={pvp?.role}
        onReportResult={async (payload) => {
          const prize = payload.prizeAwarded;
          const result = payload.matchResult;
          const rounds = payload.rounds;

          // 1) proovi RPC
          const rpc = await supabase.rpc("record_match", {
            p_user_id: userId,
            p_buy_in: buyInRef.current,
            p_prize: prize,
            p_result: result,
            p_rounds: rounds as any,
          });

          if (rpc.error) {
            // 2) fallback – otse tabelitesse
            ...
            await supabase
              ...
              .update({
                ...
                wins: (cur.wins ?? 0) + (result === "you_win" ? 1 : 0),
                losses: (cur.losses ?? 0) + (result === "you_lose" ? 1 : 0),
                total_won: (cur.total_won ?? 0) + prize,
                total_lost:
                  (cur.total_lost ?? 0) + (result === "you_lose" ? buyInRef.current : 0),
              })
              .eq("user_id", userId);
          }

          // -> anna YourStats’ile refresh signaal (töötab ka ilma Realtime’ita)
